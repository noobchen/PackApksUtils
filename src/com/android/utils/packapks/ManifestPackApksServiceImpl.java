 ...
 */
public class ManifestPackApksServiceImpl implements PackApksService {
    private final String configName = "AndroidManifest.xml";
    private String keystoreName;
    private String keystorePwd;

    public ManifestPackApksServiceImpl(String keystoreName, String keystorePwd) {
            ...
            try {
                ...
                while ((line = br.readLine()) != null) {
                    if (line.contains("\"UMENGCHANNEL\"")) {
                        line = line.replace("UMENGCHANNEL", channel.split("\\|")[2]);

                    }

                    if (line.contains("\"ZHUQUECHANNEL\"")) {
                        line = line.replace("ZHUQUECHANNEL", channel);
    ...
    private ArrayList<String> readChannels() {
        File txt = new File("channels_xxdmx.txt");

        if (!txt.exists()) {
            System.out.println("Channels.txt 不存在！");
            return null;
        }
